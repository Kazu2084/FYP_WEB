...
if (isset($_POST['btn_save'])) {
	...
	$query = "insert into staff_salary_allowances(staff_id,basic_salary,medical_allowance,hr_allowance,scale)values('$staff_id','$basic_salary','$medical_allowance','$hr_allowance','$scale')";
...
if (isset($_POST['btn_sub'])) {
	...
	$query = "select * from staff_salary_allowances where staff_id='$staff_id'";
	$run = mysqli_query($con, $query);
	while ($row = mysqli_fetch_array($run)) {
		$total_amount = $row['basic_salary'] + ($row['basic_salary'] * $row['medical_allowance'] / 100) + ($row['basic_salary'] * $row['hr_allowance'] / 100);
		$query1 = "INSERT INTO staff_salary_report(staff_id, total_amount, status) VALUES ('$staff_id','$total_amount','Paid')";
		$run1 = mysqli_query($con, $query1);

		if ($run1) { ?>
			<script type="text/javascript">
				alert("Salary has been paid to ID is : " + <?php echo $row['staff_id'] ?>);
			</script>
		<?php } else { ?>
			<script type="text/javascript">
...
<div class="app-content">
	<div class="app-content-header">
		<h1 class="app-content-headerText">Staff Salary</h1>
		...
		<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
			...
			<div class="modal-dialog modal-lg">
				...
				<div class="products-area-wrapper tableView">
					...
					<?php
					$query = "select tsr.staff_id,ti.first_name,middle_name,last_name,salary_id,basic_salary,medical_allowance,hr_allowance,scale,Date(paid_date) as paid_date,total_amount from staff_salary_allowances tsa inner join staff_salary_report tsr on tsa.staff_id=tsr.staff_id inner join staff_info ti on ti.staff_id=tsr.staff_id";
					$run = mysqli_query($con, $query);
					while ($row = mysqli_fetch_array($run)) { ?>
